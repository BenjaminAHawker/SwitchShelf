...
const { DATA_DIR, isValidFileName } = require('./sync');
...
// instead: anything above v0 is an update, v0 is always the base — compared
// numerically so "0", "00", a bare 0, null/undefined, etc. all count as base,
// not just an exact "0" string match.
function displayContentType(match, version) {
  if (!match) return null;
  const type = contentTypeOf(match);
  if (type === 'dlc' || type === 'demo' || type === 'update') return type;
  return Number(version) > 0 ? 'update' : type || 'base';
}
...
function search(name, query, { field = 'all', limit = 200, contentType = 'game', owned = 'all', languages = [], sort = 'name-asc' } = {}) {
  ...
  const matches = [];
  // titledb sometimes lists the same base game more than once under the same
  // titleId — a bundle/collector's edition alongside the plain edition, each
  // with its own nsuId and icon, with nothing to tell them apart as "the same
  // game" other than the shared id. Object.keys on a JSON object with numeric
  // keys (nsuId) is returned in ascending order, so the first one seen here is
  // always the lowest nsuId, i.e. the original/plain listing — keep that one
  // and drop any later entries sharing its id.
  ...
  for (const key of Object.keys(data)) {
    ...
    if (type !== 'dlc' && type !== 'update' && type !== 'demo') {
      const id = String(title.id).toUpperCase();
      if (seenBaseIds.has(id)) continue;
      seenBaseIds.add(id);
    }
