...
// Whether a specific catalog entry counts as "owned", accounting for ids that
// have both a Switch and a Switch 2 Edition entry sharing them — those need
// to be judged independently, so accepting one variant in Library Scan /
// Staging doesn't mark the other as owned too. acceptedVariantsByTitle is
// decisions.getAcceptedVariantsByTitle()'s Map<titleId, Set<variant>>;
// omit it to have this build one itself (fine for one-off lookups, but
// callers looping over many titles should build it once and pass it in).
function isOwned(name, title, acceptedVariantsByTitle) {
  if (!title?.id) return false;
  const id = String(title.id).toUpperCase();
  const variantsMap = acceptedVariantsByTitle || decisions.getAcceptedVariantsByTitle();
  const acceptedVariants = variantsMap.get(id);
  if (!acceptedVariants) return false;

  const siblings = findAllByTitleId(name, id);
  if (siblings.length <= 1) return true; // no Switch/Switch2 ambiguity for this id

  if (isSwitch2(title)) return acceptedVariants.has('switch2');
  // An unset variant defaults to the non-Switch2 entry (see resolveVariant),
  // so a null-variant acceptance claims the Switch entry, not both.
  return acceptedVariants.has('switch') || acceptedVariants.has(null);
}
...
function search(name, query, { field = 'all', limit = 200, platform = 'all', contentType = 'game', owned = 'all', languages = [], sort = 'name-asc' } = {}) {
  const data = loadRegion(name);
  if (!data) {
    return null;
  }
  ...
  const acceptedVariantsByTitle = decisions.getAcceptedVariantsByTitle();
  ...
  for (const key of Object.keys(data)) {
    ...
    const titleOwned = isOwned(name, title, acceptedVariantsByTitle);
    ...
    matches.push({ key, ...title, isSwitch2: switch2, contentType: type, owned: titleOwned });
  }
