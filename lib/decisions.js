...
// own. Omit (undefined) to leave any existing override untouched, same as
// variant; pass null explicitly to clear it back to the filename-derived one.
// source: 'library' (default, backward-compatible: unprefixed key) | 'staging'.
function setDecision(filePath, { status, titleId, region, variant, version }, source = 'library') {
...
// Map<titleId, Set of accepted variants ('switch' | 'switch2' | null)>.
// Complements getAcceptedTitleIds when an id has both a Switch and a Switch 2
// Edition catalog entry sharing it — those need to be judged owned
// independently of each other (accepting one shouldn't mark the other too).
function getAcceptedVariantsByTitle() {
  const decisions = readAll();
  const map = new Map();
  for (const decision of Object.values(decisions)) {
    if (decision.status !== 'accepted' || !decision.titleId) continue;
    const id = String(decision.titleId).toUpperCase();
    if (!map.has(id)) map.set(id, new Set());
    map.get(id).add(decision.variant || null);
  }
  return map;
}

// Distinct manually-set version overrides accepted for a titleId (library or
// staging), e.g. an update version the user has that cnmts.json doesn't know
// about yet. Only reflects explicit overrides (see setDecision's `version`) —
// versions merely parsed from a filename are never persisted, so they aren't
// visible here.
