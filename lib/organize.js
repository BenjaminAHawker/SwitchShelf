...
function buildPlanFor({ source, sourceDir, destDir, region }) {
  ...
  for (const [filePath, decision] of Object.entries(sourceDecisions)) {
    ...
    // version differs), so a match resolved by titleId alone always comes
    // back as the base game's own catalog entry — cnmts has no way to tell
    // "the base install" and "an update of it" apart by id. Infer it from the
    // version instead: anything other than v0 is an update, not the base.
    const displayType = type === 'dlc' || type === 'demo' || type === 'update'
      ? type
      : version && version !== '0'
        ? 'update'
        : type || 'base';
