...
// After moving a file out of a Staging subfolder, walks upward from that
// subfolder removing it (and any now-empty parents) as long as they're
// strictly inside boundaryDir — so a folder that held only that one file
// doesn't linger behind empty. Never touches boundaryDir itself, and stops
// as soon as a directory still has something in it.
function removeEmptyDirsUpward(startDir, boundaryDir) {
  let dir = startDir;
  while (isInsideDir(boundaryDir, dir)) {
    let entries;
    try {
...
function buildPlanFor({ source, sourceDir, destDir, region }) {
  ...
  for (const [filePath, decision] of Object.entries(sourceDecisions)) {
    ...
    if (!isInsideDir(sourceDir, sourceFull)) {
      skipped.push({ path: filePath, reason: 'Source path escapes the source folder' });
      continue;
    }
    ...
    const match = store.findByTitleId(region, decision.titleId);
    if (!match || !match.name) {
    ...
    if (type === 'dlc') {
      ...
      version = decision.version || scanner.extractVersion(path.basename(filePath)) || '0';
      fileName = `${sanitize(match.name, match.id)} [${match.id}][v${version}]${ext}`;
    }

    const displayType = store.displayContentType(match, version);
...
function applyPlanFor({ source, sourceDir, destDir, region, paths, crossSource }) {
  ...
  for (const item of plan) {
    ...
    try {
      if (!isInsideDir(sourceDir, sourceFull)) throw new Error('Source path escapes the source folder');
      ...
      if (crossSource) {
        ...
        // Staging is a flat drop-off/review area — if that was the last
        // file in its subfolder, clean the empty subfolder(s) up too.
        removeEmptyDirsUpward(path.dirname(sourceFull), sourceDir);
      } else {
